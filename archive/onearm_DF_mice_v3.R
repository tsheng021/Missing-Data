...
result <- parSapply(cl, 1:S, function(x){
  ...
  # MTN-MMRM
  D_mtn <- c(1, bl_bar, (N-r)/N, (r+q)/N, q/N, (r+q)/N*bl_bar)
  theta_mtn_mmrm <- D_mtn%*%coeff
  se_mtn_mmrm <- sqrt(D_mtn%*%vcov(m1)%*%D_mtn + 
                        r*(N-r)/N^3*(coeff[1]+(coeff[2]+coeff[6])*bl_bar+coeff[4])^2+
                        q*(N-q)/N^3*(coeff[1]+(coeff[2]+coeff[6])*bl_bar+coeff[3]+coeff[4]+coeff[5])^2+
                        (r+q)*(N-r-q)/N^3*(coeff[1]+coeff[2]*bl_bar+coeff[3])^2-
                        2*r*q/N^3*(coeff[1]+(coeff[2]+coeff[6])*bl_bar+coeff[4])*(coeff[1]+(coeff[2]+coeff[6])*bl_bar+coeff[3]+coeff[4]+coeff[5])-
                        2*r*(N-r-q)/N^3*(coeff[1]+(coeff[2]+coeff[6])*bl_bar+coeff[4])*(coeff[1]+coeff[2]*bl_bar+coeff[3])-
                        2*q*(N-r-q)/N^3*(coeff[1]+(coeff[2]+coeff[6])*bl_bar+coeff[3]+coeff[4]+coeff[5])*(coeff[1]+coeff[2]*bl_bar+coeff[3])
  )
  ...
  # J2B-MMRM
  D_j2b <- c((r+q)/N, (r+q)*bl_bar/N, q/N, (r+q)/N, q/N, (r+q)*bl_bar/N)
  theta_j2b_mmrm <- D_j2b%*%coeff+(N-r-q)/N*bl_bar
  se_j2b_mmrm <- sqrt(D_j2b%*%vcov(m1)%*%D_j2b +
                        (coeff[1]+bl_bar*(coeff[2]+coeff[6])+coeff[4])^2*r*(N-r)/N^3+
                        (coeff[1]+bl_bar*(coeff[2]+coeff[6])+coeff[3]+coeff[4]+coeff[5])^2*q*(N-q)/N^3+
                        bl_bar^2*(N-r-q)*(r+q)/N^3-
                        2*r*q/N^3*(coeff[1]+bl_bar*(coeff[2]+coeff[6])+coeff[4])*(coeff[1]+bl_bar*(coeff[2]+coeff[6])+coeff[3]+coeff[4]+coeff[5])-
                        2*r*(N-r-q)/N^3*(coeff[1]+bl_bar*(coeff[2]+coeff[6])+coeff[4])*bl_bar-
                        2*q*(N-r-q)/N^3*(coeff[1]+bl_bar*(coeff[2]+coeff[6])+coeff[3]+coeff[4]+coeff[5])*bl_bar
  )
  ...
  for(j in 1:MI){
    ...
    ana <- summary(l)$coef$fixed
    d_ana <- c(1, bl_bar, 1-r/N, 1, 1-r/N, bl_bar)
    theta_mtn_mi_j[j] <- d_ana%*%ana
    vartotal_mtn_mi_j[j] <- d_ana%*%vcov(l)%*%d_ana
    ...
    ana <- summary(l)$coef$fixed
    d_ana <- c(1, bl_bar, 1-r/N, 1, 1-r/N, bl_bar)
    theta_j2b_mi_j[j] <- d_ana%*%ana
    vartotal_j2b_mi_j[j] <- d_ana%*%vcov(l)%*%d_ana
  }
  # MAR
  ...
  # dt_mtn <- l[1]+l[2]*bl_bar+(l[3]-1)*mean(y_mis$t1)+l[4]
  # dt_j2b <- l[1]+(l[2]-1)*bl_bar+l[3]*mean(y_mis$t1)+l[4]
  # 
  # for(j in 1:MI){
  #   # MAR
  #   y_mar <- y_mis
  #   y_mar$t2[ind_mis] <- imputed[,j]
  #   lmi <- summary(lm(t2~t0+t1,y_mar))$coef[,1]
