...
S <- 5000
true <- (mu_trt[2]*pi + mu_act[2]*pi_c + mu_mis[2]*(1-pi-pi_c))-
  (eta_pla[2]*omega + eta_act[2]*omega_c + eta_mis[2]*(1-omega-omega_c))
no_cores <- detectCores()-1
cl <- makeCluster(no_cores)
clusterExport(cl,varlist=c('S', 'N', 'vv', 'true', 'alpha',
...
result <- parSapply(cl, 1:S, function(x){
  ...
  low_obs <- diff_obs - qt(1-alpha/2, q-1)*se_obs
  upp_obs <- diff_obs + qt(1-alpha/2, q-1)*se_obs
