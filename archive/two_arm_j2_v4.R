...
result <- parSapply(cl, 1:S, function(x){
  ...
  pval_obs <- t.test(y_obs$t2, z_obs$t2)$p.value<.05
  ...
  ## MI Rubin ##
  MI <- 10
  z <- matrix(c(1,0,1,0,0,0, 1,1,1,0,1,0), nrow=2, byrow=T)
  mu_act <- z%*%t(t(coef(summary(m1))[,1]))
  
  z <- matrix(c(1,1,0,1), nrow=2)
  sigma_mu <- z%*%getVarCov(m1)%*%t(z)/(n1_act+n2_act-1)
  
  diff_mi_j <- vartotal_mi_j <- rep(NA, MI)
  for(j in 1:MI){
    mu1_act_obs<- mean(c(y_mis$t1, z_mis$t1))
    mu2_act_tilde <- rnorm(1, mu_act[2]+sigma_mu[1,2]/sigma_mu[1,1]*(mu1_act_obs-mu_act[1]),
                           sqrt(sigma_mu[2,2]-sigma_mu[1,2]/sigma_mu[1,1]*sigma_mu[1,2]))
    
    sig_tilde2 <- rinvchisq(1, 2*N-n1_mis-n2_mis-1)*m1$sigma^2*(2*N-n1_mis-n2_mis-1)
