...
for(i in 1:S){
  ...
  while (sum(ind==3)==0) ind[ind==2]<-ind[ind==2] + rbinom(sum(ind==2), 1, pt)
  ind_mis <- which(ind==3)
...
result <- parSapply(cl, 1:S, function(x){
  ...
  theta_mar_mi_j <- vartotal_mar_mi_j <- 
    theta_reg_mi_j <- vartotal_reg_mi_j <-
    rep(NA, MI)
  for(j in 1:MI){
    y_mis$t2[ind_mis] <- NA
    for(k in 1:N){
      if(is.na(y_mis$t2[k])){
        ...
        mu_act <- z_act %*% coeff
        mu_act_tilde <- mvrnorm(1, mu_act, sigma/(N-r))
        sigma_tilde <- matrix(rWishart(1, N-r-1, sigma),nrow=2)/(N-r-1)
        y_mis$t2[k] <- rnorm(1,
    ...
    theta_mar_mi_j[j] <- mean(y_mis$t2)
    vartotal_mar_mi_j[j] <- var(y_mis$t2)/N
  }
  # MAR
  ...
  for(j in 1:MI){
    y_mis$t2[ind_mis] <- imputed[,j]
    theta_reg_mi_j[j] <- mean(y_mis$t2)
    vartotal_reg_mi_j[j] <- var(y_mis$t2)/N
  }
  theta_reg_mi <- mean(theta_reg_mi_j)
  W <- mean(vartotal_reg_mi_j)
  B <- var(theta_reg_mi_j)
  se_reg_mi <-  sqrt(W + (1+1/MI)*B)
  df <- (1+MI/(MI+1)*W/B)*(MI-1)
  low_reg_mi <- theta_reg_mi - qt(1-alpha/2, df)*se_reg_mi
  upp_reg_mi <- theta_reg_mi + qt(1-alpha/2, df)*se_reg_mi
  cover_reg_mi <- low_reg_mi<true_df & true_df<upp_reg_mi
  out <- c(out, theta_reg_mi, se_reg_mi, cover_reg_mi)
})
